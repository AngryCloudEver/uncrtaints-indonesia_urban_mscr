...
def create_parser(mode='train'):
    ...
    # parser.add_argument("--experiment_name", default='monotemporalL2', help="Name of the current experiment",)
    parser.add_argument("--experiment_name", default='monotemporalL2', help="Name of the current experiment",)
    ...
    parser.add_argument("--n_head", default=1, type=int, help="default value of 16, 4 for debugging")
    parser.add_argument("--d_model", default=256, type=int, help="layers in L-TAE, default value of 256")
    parser.add_argument("--positional_encoding", default=True, dest="positional_encoding", action="store_false", help="whether to use positional encoding or not") 
    parser.add_argument("--d_k", default=4, type=int)
    parser.add_argument("--low_res_size", default=32, type=int, help="resolution to downsample to")
    parser.add_argument("--use_v", default=False, dest="use_v", action="store_true", help="whether to use values v or not")
    ...
    # training parameters
    parser.add_argument("--loss", default="l2", type=str, help="Image reconstruction loss to utilize [l1|l2|GNLL|MGNLL].")
    parser.add_argument("--resume_from", default=False, dest="resume_from", action="store_true", help="resume training acc. to JSON in --experiment_name and *.pth chckp in --trained_checkp")
    ...
    parser.add_argument("--lr", default=0.001, type=float, help="Learning rate, e.g. 0.01")
    parser.add_argument("--gamma", default=0.8, type=float, help="Learning rate decay parameter for scheduler")
    ...
    parser.add_argument("--use_sar", default=True, dest="use_sar", action="store_true", help="whether to use SAR or not")
    parser.add_argument("--pretrain", default=True, dest="pretrain", action="store_true", help="whether to perform pretraining on SEN12MS-CR or training on SEN12MS-CR-TS") 
    parser.add_argument("--input_t", default=1, type=int, help="number of input time points to sample, unet3d needs at least 4 time points")
    parser.add_argument("--ref_date", default="2014-04-03", type=str, help="reference date for Sentinel observations")
    parser.add_argument("--sample_type", default="pretrain", type=str, help="type of samples returned [cloudy_cloudfree | generic]")
    parser.add_argument("--vary_samples", default=True, dest="vary_samples", action="store_false", help="whether to sample different time points across epochs or not") 
    ...
    parser.add_argument("--trained_checkp", default="results/monotemporalL2/model.pth.tar", type=str, help="Path to loading a pre-trained network *.pth file, rather than initializing weights randomly")
    ...
    parser.add_argument("--scale_by", default=10.0, type=float, help="rescale data within model, e.g. to [0,10]")
    parser.add_argument("--separate_out", default=False, dest="separate_out", action="store_true", help="whether to separately process mean and variance predictions or in a shared layer")
    ...
    parser.add_argument("--use_custom", default=True, dest="use_custom", action="store_true", help="whether to test on individually specified patches or not")
